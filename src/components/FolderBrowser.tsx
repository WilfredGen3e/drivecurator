...
export default function FolderBrowser({ msalInstance, account, onBack, onFolderSelected }: Props) {
  ...
  // Toon de laatste sessie alleen als die verschilt van de favoriete map
  ...
  return (
    <div className="max-w-xl mx-auto px-6 py-8 space-y-5">
      ...
      {(favorite || showLastFolder) && (
        <div className="border border-fluent-border bg-fluent-bg-primary">
          <div className="px-4 py-1.5 bg-fluent-bg-secondary border-b border-fluent-border">
            <span className="text-xs font-semibold text-fluent-text-secondary uppercase tracking-wider">Snelkoppelingen</span>
          </div>
          {favorite && (
            <div className="flex items-center gap-3 px-4 py-2.5 border-b border-fluent-border last:border-b-0">
              ...
              <button
                ...
                className="flex-shrink-0 flex items-center gap-1.5 px-3 py-1 bg-fluent-accent hover:bg-fluent-accent-hover text-white text-xs font-semibold transition-colors"
          ...
          {showLastFolder && (
            <div className="flex items-center gap-3 px-4 py-2.5">
              ...
              <button
                ...
                className="flex-shrink-0 flex items-center gap-1.5 px-3 py-1 border border-fluent-border-strong text-fluent-text-secondary hover:bg-fluent-bg-hover text-xs font-semibold transition-colors"
                style={{ borderRadius: 2 }}
              >
        ...
        <div className="border border-fluent-border">
          ...
          {folders.map((folder, i) => {
            ...
            return (
              <div
                key={folder.id}
                className={`flex items-center gap-3 px-4 py-2.5 hover:bg-fluent-bg-hover transition-colors ${i < folders.length - 1 ? 'border-b border-fluent-border' : ''}`}
              >
                <button
                ...
                <button
                  ...
                  className={`flex-shrink-0 p-1 transition-colors ${isFav ? 'text-amber-400 hover:text-amber-500' : 'text-fluent-text-disabled hover:text-amber-400'}`}
                >
                  <svg className="w-4 h-4" fill={isFav ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth={isFav ? 0 : 1.5} viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                  </svg>
                </button>
                <button onClick={() => handleFolderClick(folder)} className="flex-shrink-0 text-fluent-text-disabled">
                  <ChevronIcon />
                </button>
              </div>
            )
