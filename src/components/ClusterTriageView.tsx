...
import { DriveItem, deleteItem } from '../services/graphService'
...
export default function ClusterTriageView({ msalInstance, account, clusterLabel, initialPhotos, onDone }: Props) {
  ...
  const deletedCount = initialPhotos.length - total
  ...
  const handleTouchEnd = () => {
    ...
    if (!wasActive) return
    if (Math.abs(x) >= SWIPE_COMMIT && Math.abs(x) >= Math.abs(y)) {
      if (x < 0) handleDelete()
      else handleNext()
    }
  }
  ...
  const swipeHoriz = swipeAbsX >= Math.abs(swipeDelta.y)
  ...
  const photoSwipeTransform = (() => {
    if (!isActivelySwiping || !swipeHoriz) return 'none'
  ...
  // Klaar: alle foto's gezien of verwijderd
  if (!photo) {
    return (
      <div className="h-full flex flex-col items-center justify-center gap-4 text-center px-6 bg-fluent-bg-secondary">
        ...
        <div>
          ...
          {deletedCount > 0 && (
            <p className="text-fluent-text-secondary text-sm mt-1">{deletedCount} foto{deletedCount !== 1 ? "'s" : ''} verwijderd</p>
          )}
        </div>
      ...
      <div className="flex flex-col h-full bg-fluent-bg-secondary">
        ...
        >
          ...
          {isActivelySwiping && swipeDelta.x < -SWIPE_HINT && (
            <div className="absolute inset-0 flex items-center justify-end pr-10 pointer-events-none transition-colors duration-100"
              style={{ backgroundColor: swipeLeftCommitted ? 'rgba(209,52,56,0.85)' : `rgba(209,52,56,${Math.min(0.25, swipeAbsX / SWIPE_COMMIT * 0.25)})` }}>
          ...
          {isActivelySwiping && swipeDelta.x > SWIPE_HINT && (
            <div className="absolute inset-0 flex items-center justify-start pl-10 pointer-events-none transition-colors duration-100"
              style={{ backgroundColor: swipeRightCommitted ? 'rgba(16,124,16,0.85)' : `rgba(16,124,16,${Math.min(0.25, swipeDelta.x / SWIPE_COMMIT * 0.25)})` }}>
        ...
        {/* Toast */}
        {toast && (
          <div className="fixed bottom-24 left-1/2 -translate-x-1/2 bg-fluent-text-primary text-white text-sm px-4 py-2 z-50" style={{ borderRadius: 2 }}>
            {toast}
          </div>
        )}
      </div>
  ...
  // ── Desktop layout ────────────────────────────────────────────────────────
  return (
    <div className="flex flex-col h-full bg-fluent-bg-primary">
      {/* Topbalk */}
      <div className="flex items-center gap-2 px-3 border-b border-fluent-border flex-shrink-0 h-10">
        <button onClick={() => onDone(photos)} className="text-fluent-text-secondary hover:text-fluent-text-primary text-sm transition-colors flex items-center gap-1">
...
function TouchBtn({ onClick, disabled, label, color, children }: {
  ...
  color: 'danger' | 'success' | 'secondary'; children: React.ReactNode
}) {
  const styles = {
